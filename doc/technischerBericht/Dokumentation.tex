...
\begin{document}
  ...
  \ac{REST} stammt aus der Dissertation von Roy Fielding\cite{REST} in der er
  den Erfolg des \ac{WWW} auf bestimmte Eigenschaften der verwendeten Mechanismen und
  ...
  hinaus wesentlich performanter als \ac{XML} basierte Protokolle, wie z.B.
  \ac{SOAP} oder XML-RPC.
  ...
  \ac{REST} ist in aller Munde. Seit gut drei Jahren höre ich immer wieder, wie
  einfach dass eine Schnittstelle mit \ac{REST} zu implementieren ist. Das mag
  auch sein, wenn man sich an einen Standard wie JSR-000311\cite{JSR311} hält, und
  ...
  ihren Systemen über \ac{REST} bereitstellen, was den Akzeptanzfaktor in einem
...
\end{verbatim}  
  ...
  Wachstum der Sourcen anhand der versionierten Commits verfolgen, auch
  ...
  \begin{itemize}
    ...
    \item www.github.com, für die Verwaltung des Sourcecodes
  ...
  \begin{itemize}
    ...
    mitgeliefert
    \item \ac{JPA} 2.0 für die Abstraktion des Persistenzzugriffs, wird mit dem
    GlassFish Server mitgeliefert
    \item \ac{EJB} 3.1, für die Verwaltung von Enterprise Beans, wird mit dem
    GlassFish Server mitgeliefert
    \item Jersey, für den \ac{REST} Webservice, wird mit dem GlassFish Server
    mitgeliefert
  ...
  Die detaillierte Umsetzung ist im Sourcecode nachzuvollziehen. Ich beschränke
  mich hier auf einen groben Überblick.
  
  \subsection{REST Client}
  
  Als erstes habe ich mich an den \ac{REST} Client auf Android gewagt. Ich
  wollte mich so schnell wie möglich in das Thema Android einarbeiten. Dabei
  habe ich die Entwicklung gegen einen bestehenden RESTful Webservice auf Ruby
  on Rails Basis gemacht. 
  
  Schnell habe ich festgestellt, dass ich auf der Androidplattform kein
  \ac{JAXB} Framework zur Verfügung habe, auch existierte keine Möglichkeit die
  Referenz Implementierung für einen \ac{REST} Client von JAVA, namens Jersey,
  in die Androidplattform einzubinden. Das Problem dabei war die Dalvik Virtual
  Machine\cite{Dalvik}, auf die die Androidplattform aufbaut. Sie akzeptiert
  nur ein Teil aller Java Packages. Wenn eine Abhängigkeit zu einem Java
  Package besteht, das nicht unterstützt wird, muss man einen anderen Weg
  finden, um zum Ziel zu kommen.
  
  Für das Objektmapping und die Kommunikation mit einer \ac{REST} Schnittstelle
  ...
  Nun habe ich noch ein kleines \ac{GUI} für die Ansteuerung der \ac{REST} Funktionen
  gebaut.
  
  \subsection{REST Server}
  
  Nach einer erfolgreichen Umsetzung des Clients habe ich mich an den
  Serverteil gemacht. Ich habe die \ac{REST} Schnittstelle strikte nach dem
  Konzept von Jersey Implementiert. Bei der Lösung habe ich mich vom Buch
  ``Real World Java EE Patterns''\cite{RWJavaEEP} inspirieren lassen. Zudem
  habe ich einen Persistenzlayer für den Zugang zur Datenbank geschaffen. Um
  die \ac{REST} Funktionen einfach zu testen, habe ich auch ein kleines
  \ac{GUI} auf Basis von Java Server Faces 2.0 programmiert. In der Grafik
  \ref{restPrinzip} ist ersichtlich wie die \ac{REST} Funktionen anzusprechen
  sind.
  
  Die serverseitige Konfiguration beschränkt sich dabei auf eine Javaklasse,
  in der der Einstiegspunkt zum \ac{REST} Service definiert ist. Diese Klasse sieht
  so aus:
...
\end{verbatim}
  ...
  @ApplicationPath(''''), dass \ac{REST} Requests an die Jersey implementierung
  weitergereicht werden sollen, sofern eine existiert.
  
  \subsection{Hessian Server}
  
  Den Hessian Service habe ich ganz nach der Anleitung der Hessian
  Dokumentation\cite{Hessian} implementiert. Der Service beschränkt sich auf
  eine Klasse namens ProductRegistrationServiceEndpoint welche von der Klasse
  HessianServlet ableitet und das Interface der Schnittstelle implementiert,
  siehe Grafik \ref{hessianService}. Da ich bereits ein Persistenzlayer zur
  Datenbank geschaffen habe, konnte ich diesen wieder verwenden, um
  Datenobjekte zu persistieren, zu aktualisieren, zu löschen oder zu laden.
  ...
  Die Konfiguration ist mit zwei Einträgen im web.xml File gemacht, siehe Grafik
  \ref{hessianConfig}.
